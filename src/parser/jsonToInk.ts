import { InkBook, InkNode, InkChapter, InkScene, InkTextNode, InkBlockNode } from './types';

export function jsonToInk(ast: any): { result: string; errors: string[] } {
  let result = '';
  const errors: string[] = [];
  ...
  try {
    ...
    function renderNode(node: InkNode): string {
      let r = '';
      if (node.type === 'TEXT') {
         r += (node as InkTextNode).content + '\n\n';
      } else if (node.type === 'THOUGHT') {
         r += `[[THOUGHT]]\n`;
         (node as InkBlockNode).children.forEach(c => r += renderNode(c));
         r += `[[ENDTHOUGHT]]\n\n`;
    ...
    errors.push(`JSON to INK Mapping Error: ${(err as Error).message}`);
  }
